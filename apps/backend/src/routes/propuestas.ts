...
const adminInclude = { user: { select: { id: true, displayName: true, email: true } }, logro: true } as const
...
router.get("/admin/propuestas", authMiddleware, requireTeamAdmin, async (req, res) => {
  ...
  res.json(await prisma.propuestaLogro.findMany({ where: { teamId: req.team!.id, ...(status.data === "all" ? {} : { status: status.data }) }, include: adminInclude, orderBy: [{ createdAt: "asc" }, { id: "asc" }] }))
})

router.get("/admin/propuestas/:id", authMiddleware, requireTeamAdmin, async (req, res) => {
  const proposal = await prisma.propuestaLogro.findFirst({ where: { id: proposalId(req.params.id), teamId: req.team!.id }, include: adminInclude })
  if (!proposal) throw new AppError(404, "Propuesta no encontrada")
  res.json(proposal)
})
...
router.post("/admin/propuestas/:id/aceptar", authMiddleware, requireTeamAdmin, validate(aceptarPropuestaSchema), async (req, res) => {
  ...
  const proposal = await prisma.$transaction(async tx => {
    ...
    const logro = await tx.logro.create({ data: { teamId: pending.teamId, nombre: pending.nombre, descripcion: pending.descripcion, criterios: pending.criterios, puntos: req.body.puntos, categoria: req.body.categoria } })
    // 📚 No se escribe UserLogro ni SolicitudLogro: la autoría no equivale a haber obtenido el logro.
    return tx.propuestaLogro.update({ where: { id }, data: { logroId: logro.id }, include: adminInclude })
  })
  res.json(proposal)
})
...
router.post("/admin/propuestas/:id/rechazar", authMiddleware, requireTeamAdmin, validate(rejectAchievementRequestSchema), async (req, res) => {
  ...
  res.json(await prisma.propuestaLogro.findUniqueOrThrow({ where: { id }, include: adminInclude }))
})
